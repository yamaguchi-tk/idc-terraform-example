...
### DeveloperAccess（inline policy を付与するサンプル）
### DeveloperAccess (sample of attaching an inline policy)
...
resource "aws_ssoadmin_managed_policy_attachment" "DeveloperAccess" {
  instance_arn       = local.instance_arn
  managed_policy_arn = "arn:aws:iam::aws:policy/ReadOnlyAccess"
  permission_set_arn = aws_ssoadmin_permission_set.DeveloperAccess.arn
}

resource "aws_ssoadmin_permission_set_inline_policy" "DeveloperAccess" {
  instance_arn       = local.instance_arn
  permission_set_arn = aws_ssoadmin_permission_set.DeveloperAccess.arn
  ...
  inline_policy = jsonencode({
    ...
    Statement = [
      {
        Sid    = "AllowS3ReadWriteForDeveloperBucket"
        Effect = "Allow"
        Action = [
          "s3:GetObject",
          "s3:PutObject",
        ...
        Resource = [
          "arn:aws:s3:::example-developer-bucket",
          "arn:aws:s3:::example-developer-bucket/*",
        ]
      }
    ]
